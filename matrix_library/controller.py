...
class Controller:
                    ...
                    for device in devices:
                        ...
                        if re.search("8BitDo Zero 2 gamepad", device.name):
                            if self.gamepad is None:
                                ...
                                logging.info(
                                    f"Two controllers already mapped. Skipping {device.path} - {device.name}"
                                )
                except:
    ...
    def add_function(self, button, function):
        if mode == "board":
            ...
            logging.debug(
                f"add_function: Key: {button}, {self.function_map[button]['function']}"
            )
            ...
            logging.warning(
                f"Unhandled mode: {mode}, button: {button}, function: {function}"
            )
...
class Workstation_listener:
    ...
    def start(self):
        if self.running == False:
            ...
            self.listener = pynput.keyboard.Listener(on_press=self.on_press_handler)
            self.listener.start()
    ...
    def on_press_handler(self, key):
        try:
            if (
                isinstance(key, pynput.keyboard.KeyCode)
                and self.controller.execution_map[key.char]
            ):
